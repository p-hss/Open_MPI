...
!     This program sums all rows in a vector using MPI parallelism.
!     The root process acts as a master and sends a portion of the
!     vector to each child process.  Master and child processes then
!     all calculate a partial sum of the portion of the vector assigned
...
!     the master, who calculates a grand total.
      ...
      integer num_procs, an_id, num_rows_to_receive, start_row, end_row
...
!        distribute a portion of the vector to each child process,
         ...
         do an_id = 1, num_procs -1
            ...
            print *, "send number of rows", num_rows_to_send, " send to ", an_id 

            call MPI_SEND( vector(start_row), num_rows_to_send, MPI_REAL, &
           an_id, send_data_tag, MPI_COMM_WORLD, ierr)
            
            
            print *, "send data", vector(start_row), " send to ", an_id 
...
!        I must be a slave process, so I must receive my vector segment,
!        storing it in a "local" vector, vector2.  
