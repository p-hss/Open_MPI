program main 
    ...
    integer my_id, root_process, num_procs, ierr, i, num_intervals
    ...
    !This is the root process. ask for number of intervalls
    ...
    !For any porcess do the following:     
    !Broadcast the number of intervals send to every process, from root.

    call MPI_BCAST (num_intervals, 1, MPI_INTEGER, root_process, MPI_COMM_WORLD, ierr)

    !calculate the width of a rectangle, pi = total length of integration domain.

    rect_width = pi / num_intervals

    !then calculate the sum of the areas of the rectangles.
    !Start with the (my_id +1)th interval and process every
    !num_procs-th interval thereafter.
    ...
    print *,"proc [id]", my_id, "computes:", partial_sum

    !and finally, engage in a reduction in which all partial sums 
    !are combined, and the grand sum appears in variable "sum" in
    !the root process,

    call MPI_REDUCE (partial_sum, sum, 1, MPI_DOUBLE_PRECISION, MPI_SUM, root_process, MPI_COMM_WORLD, ierr)
